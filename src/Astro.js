...
export const rms_vel = (molecular_weight, exospheric_temp) => {
  return Math.sqrt((3.0 * MOLAR_GAS_CONST * exospheric_temp / molecular_weight)
		   * CM_PER_METER);
}
...
/*--------------------------------------------------------------------------*/
export const vol_inventory = (mass, escape_vel, rms_vel, stellar_mass, zone, greenhouse_effect, accreted_gas) => {
...
export const hydro_fraction = (volatile_gas_inventory, planet_radius) => {
  const temp = (0.71 * volatile_gas_inventory / 1000.0) * pow2(KM_EARTH_RADIUS / planet_radius);
...
/*--------------------------------------------------------------------------*/
export const cloud_fraction = (surf_temp, smallest_MW_retained, equat_radius, hydro_fraction) => {
...
export const green_rise = (optical_depth, effective_temp, surf_pressure) => {
  const convection_factor = EARTH_CONVECTION_FACTOR * Math.pow(surf_pressure / EARTH_SURF_PRES_IN_MILLIBARS, 0.4);

  const rise = (pow1_4(1.0 + 0.75 * optical_depth) - 1.0) * effective_temp * convection_factor;
...
/*--------------------------------------------------------------------------*/
export const planet_albedo = (water_fraction, cloud_fraction, ice_fraction, surf_pressure) => {
  let rock_fraction;
...
export const calculate_surface_temp = (planet, first, last_water, last_clouds, last_ice, last_temp, last_albedo) => {
  let effective_temp;
  let water_raw;
  let clouds_raw;
  let greenhouse_temp;
  ...
  if((planet.high_temp >= planet.boil_point) && (!first) && !(planet.day == (planet.orb_period * 24.0) || (planet.resonant_period))) {
...
export const set_temp_range = planet => {
  ...
  const tiltmod  = fabs(cos(planet.axial_tilt * PI/180) * Math.pow(1 + planet.e, 2));
